       subroutine clima(ncouple,SOLCON,IRCHOICE,ICOUPLE,dt0,NSTEPS,
     & dtmax,TSTOP,TIME)
c       
...
c A. Zsom: initiating the temperature and water profile when the code is called first
      IF ((IUP.EQ.0).and.(NCOUPLE.EQ.1)) THEN
...
c give the stratosphere water mixing ratio here:
      ...
      ELSE IF ((IUP.EQ.0).and.(NCOUPLE.GT.1)) THEN
        	DO J = 1,ND
         		READ(41,*) T(J),FSAVE(J)
        	END DO
         	close (41)
...
c here we initialize the ozone profile 
      IF((ICOUPLE.EQ.1).and.(NCOUPLE.EQ.1)) THEN
c A. Zsom: reading the input_comp.dat file



c A. Zsom	
c give the ozone mixing ratio here (FI(4,J))
		do j=1,ND
			FI(1,J)=FSAVE(J)			
			FI(4,J)=FO3			
		enddo
...
c IR and SOLAR fluxes (erg/cm^2/s)
      IF (NST .EQ. NSTEPS) write(*,*)"FDNIR, FUPIR,FDNSOL,FUPSOL"
      DO 31 J=1,ND
      ...
      IF(NST.EQ.NSTEPS) write(*,*)FDNIR(J),FUPIR(J),FDNSOL(J),FUPSOL(J)
  31  CONTINUE
...
c   Units erg/K/cm^2 
      ...
      IF(NST.EQ.1) write(*,*) "Heat Capacities CPNT"
      IF(NST.EQ.1) write(*,*) CPNT
      IF(NST.EQ.1) write(*,*) "NEW TEMP"
      IF(NST.EQ.NSTEPS) write(*,*) "Heat Capacities CPNT"
      IF(NST.EQ.NSTEPS) write(*,*) CPNT
      IF(NST.EQ.NSTEPS) write(*,*) "NEW TEMP"
C New temperature calculation for all layers from radiative equilibrum
      ...
      IF(NST.EQ.1) write(*,*) TN(J),TCOOL(J),THEAT(J),G,dt0
      IF(NST.EQ.NSTEPS) write(*,*) TN(J),TCOOL(J),THEAT(J),G,dt0
  41  CONTINUE
