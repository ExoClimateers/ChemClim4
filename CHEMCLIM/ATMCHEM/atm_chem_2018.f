        ...
        subroutine atm_chem(ncouple,FLUXFAC,ICOUPLE,TSTOP,
     &		DT,NSTEPS,TIME)
...
c
c     The code is mostly writtenplanet.dat in f77 but is compiled in f90 and it 
c     contains some f90 features.
c
C     This program was created from PRESO3.F in June, 1995, for the
C     purpose of calculating ozone levels onplanets around stars of
C     different spectral types. It differs from PRESO3 by includingf 
C     NO3, N2O5, and CL2O2, and by not including the anthropogenic
C     CFC's. Sulfur chemistry was added by Alex Pavlov.
c 
c     The code was modified by Kara Krelove (3/20/02) to run cou+pled 
c     with a climate model (ICOUPLE=1). 
c     
...
c      PARAMETER(NSP=55, NSP1=NSP+1, NSP2=NSP+2, NMAX=70)
       ...
       INCLUDE 'INCLUDECHEM/parIUP.inc'
     ...
     3  USAVE(NQ,NZ),R(NZ),U(NQ),ZKM(NZ)
      DIMENSION DPU(NZ,3),DPL(NZ,3)
      DIMENSION TA(NZ),TB(NZ),TC(NZ),TY(NZ)                                    
      DIMENSION TSAV(NZ)
      DIMENSION water(NZ),FLOW(NQT),fluxsave(108),sfxsave(10)
c A. Zsom: edheight is added, becuase edorig is defined at every km,
c		but we use delz NE 1km, so we need to interpolate.
      dimension edorig(100),edheight(100)
c A. Zsom: initial values for the corresponding arrays
c values read in from input_comp.dat
      real RPA, AERSO, WFAL,SO4AE

      CHARACTER :: STARR*3,DIRDATA*12, AA*11,dircoup*14
      CHARACTER*30 CHEM(5,NR),PRODRX(NSP,NR),LOSSRX(NSP,NR)


c  common file for diagnostics in coupled mode
...
c in comDIRP.inc
      ...
      INCLUDE 'INCLUDECHEM/comPLANETBLOK.inc'
...
c A. Zsom
c      DATA T/288.15, 281.65, 275.15, 268.66, 262.17,
c     &       255.68, 249.19, 242.70, 236.21, 229.73,
c     &       223.25, 216.77, 216.65, 216.65, 216.65,
c     &       216.65, 216.65, 216.65, 216.65, 216.65,
...
c Zsom - this is for non-M dwarf stars
C ***** DEPOSITION VELOCITIES (NQ)*****
      DATA VDEP/0.2, 1., 0., 1., 1., 0.2, 0.1, 1., 3*0., 0.2, 1.,
     2  0., 2*0.2, 6*0.5, 0., 0.5, 1., 3*0.5, 0.02, 1, 3.E-4, 
     3  3*1./
c SR just CO and H2  set to 2.4e-4 and 1.2e-4 for Ep3
c      DATA VDEP/0.2, 1., 0., 1., 1., 0.2, 0.1, 1.,2.4e-4,0.,1.2e-4,0.2,
c     2  1.0,0., 2*0.2, 6*0.5, 0., 0.5, 1., 3*0.5, 0.02, 1, 3.E-4, 
c     3  3*1./
c Deposition velocities used for planets around quiescent M dwarfs and AD Leo
c      DATA VDEP/0.2, 1., 0., 1., 1., 0.2, 0.1, 1.,2.4e-4,0.,1.2e-4, 0.2,
c     2  1.,0., 2*0.2, 6*0.5, 0., 0.5, 1., 3*0.5, 0.02, 1, 3.E-4, 
c     3  3*1./
C
C ***** LOWER BOUNDARY CONDITIONS (NQT)*****
c Zsom: outgassing, or constant mixing ratios
...
c       DATA LBOUND/2*0, 1, 5*0, 1, 1, 1, 2*0, 1, 5*0, 3*0, 1, 12*0/
c fixed fluxes for non-M dwarfs
       DATA LBOUND/2*0, 1, 5*0, 2, 2, 2, 2*0, 2, 5*0, 3*0, 2, 12*0/
c SR CO & H2 fixed vdep but otherwise fixed fluxes Epoch 3 
...
c-as it MUST be read for all the cases  
      ...
      OPEN(unit=64,file= DIRDATA//'/eddy-orig.pdat',status='old')

c Files with the input parameters
c NOTE: Check this two before runnig the program
       OPEN(unit=65,file= dircoup//'/input_atmchem.dat')
...
c-as I has the same format as atm_composition.out   
      OPEN(unit=67, file= DIRINOUT//'/atm_composition.dat') 
...
c Only used when ICOUPLE=1
      OPEN(unit=71,file= dircoup//'/fromClima2Photo.dat') 
...
c Only used when ICOUPLE=1    
      OPEN(unit=72,file= dircoup//'/mixing_ratios.dat')
c A. Zsom: input_comp has the initial atmospheric compositions for chemicals that are not
c		defined in the mixing_ratios.dat file, but are necessary to fix.
c		the file is generated by run_chemclim.py
      OPEN(unit=74,file= dircoup//'/input_comp.dat')
...
C **** OUTPUT FILES
      OPEN(UNIT=214,FILE= DIRINOUT//'/rxtot.out.dat')
      OPEN(UNIT=215,FILE= DIRINOUT//'/int.rates.out.dat')
...
c      OPEN(unit=82,file= DIRINOUT//'/wave_means.dat')

c This file contains the altitude in cm vs the ozone number density (cm^-3)
c      OPEN(unit=83,file= DIRINOUT//'/O3numdens.out')
...
c  To be used as input of the climate model, formerly called Pass2SurfMP.dat
      OPEN(unit=84,file= dircoup//'/fromPhoto2Clima.dat')
...
c	 86	 DIRINOUT/h20mixing.out   (subroutine OUTPUTP)
c=================================================================
...
C     TSTOP = TIME AT WHICH CALCULATION IS TO STOPC     NSTEPS = NUMBER OF TIME STEPS TO RUN (IF TSTOP IS NOT REACHED)

c A. Zsom: the first 3 lines were removed from input_atmchem.dat
c      read(65,555)
      read(65,*)AA,ZY
      read(65,*)AA,AGL
      read(65,*)AA,ISEASON
...
C-AP
C A. Zsom: modified the format for better compatibility with my input files
...
c A. Zsom: USOL(NQ,NZ), where NQ is the number of species for which the diffusion equation is solved,
c 	NZ is the number of vertical layers, T(NZ), EDD(NZ), DEN(NZ), SO4AER(NZ), AERSOL(NZ,3), 
c	WFALL(NZ,3), RPAR(NZ,3)
c gicing initial conditions for NCOUPLE.eq.1
        WRITE(*,*) 'NCOUPLE=',NCOUPLE,'ICOUPLE=', ICOUPLE
        WRITE(*,*) 'IUP (0 - previous solution, 1 - initial)',IUP

c If we have the same number of layers, then read in previous run
	IF ((NCOUPLE.EQ.1).AND.(ICOUPLE.EQ.1).AND.(IUP.eq.0)) then
               write(*,*)'NZ',NZ,'IUP',IUP
                WRITE(*,*)"Reading from previous solution."
...
c                write(*,*)"atm O3old",O3OLD
		
           close(72)
           close(67)

c If we are not on the first coupled run, read in from the previous solution               
	else if ((ICOUPLE.EQ.1).and.(NCOUPLE.GT.1)) then
	      	READ(67,400) USOL,T,EDD,DEN,SO4AER,AERSOL,WFALL,RPAR
                WRITE(*,*) '2nd, 3rd...iter, NCOUPLE= ',NCOUPLE
 400  FORMAT(1P8E12.5)
	      	close(67)	
...
c                write(*,*)"atm O3old",O3OLD
        ...
        else if ((NCOUPLE.EQ.1).AND.(ICOUPLE.EQ.1).and.(IUP.eq.1)) then
c SR	if ((NCOUPLE.EQ.1).AND.(ICOUPLE.EQ.1)) then

c reading input_comp.dat
		read(74,*)FH2O
		read(74,*)FO3
		read(74,*)FH2
		read(74,*)FCO
...
c reading the mixing_ratios.dat
	        READ(72,*) FAR                  !Argon
		READ(72,*) FCH4			!Methane
		READ(72,*) FCO2			!Carbon dioxide	
		READ(72,*) FO2			!Oxygen	
		...
		close(72)
      		...
      		DO i = 1, NZ
        		...
        		USOL(LCH4,i) = FCH4
        	 	USOL(LCO,i) = FCO
        	  	USOL(LN2O,i) = FN2O
        	   	USOL(LCH3CL,i) =FCH3CL
			USOL(LO3,i)=FO3
       		END DO
		
		DO i=1,NQ
			DO j=1,NZ
				if (USOL(i,j).eq.0d0) then
					USOL(i,j)=Fother
				endif
			enddo
		enddo
...
c 400  FORMAT(1P8E12.5)
		...
		close(72)
           ...
           write(*,*) "No option in atm_chem.f for current situation"
           write(*,*) 'ICOUPLE:',ICOUPLE,'NCOUPLE:',NCOUPLE,'IUP:',IUP,
     &       'NZ:',NZ,'old NZ:',NZold
          STOP
	endif
c END of first IF statement from line 667
c IF you want to run only one model at a time, read in previous solution
c Not sure this flag still works
        if (ICOUPLE .eq. 0) then
           write(*,*)'Code is not coupled, reading in previous solution'
           READ(67,400) USOL,T,EDD,DEN,SO4AER,AERSOL,WFALL,RPAR
                READ(72,*) FAR       !Argon
		READ(72,*) FCH4			!Methane
		READ(72,*) FCO2			!Carbon dioxide	
...
c A. Zsom - used above
c        READ(72,*) FAR                  !Argon
c	READ(72,*) FCH4			!Methane
c	READ(72,*) FCO2			!Carbon dioxide	
c	READ(72,*) FO2			!Oxygen	
c	READ(72,*) JTROP		!Tropopause layer
c        READ(72,*) O3OLD                !Former O3 column depth

      ZTROP = (JTROP-0.5)*DELZ
      print*,'JTROP,ZTROP',JTROP,ZTROP
      endif
      close(71)
c      close(72)
c Setting the eddy diffusion coefficient
c A. Zsom: eddy_orig.pdat contains the eddy coeff. at every km, but
c		we use DELZ NE 1km, so we need to interpolate to the height grid
        do J=1,100
         	read(64,*)edorig(J),edheight(J)
c edorig assumes that the troposphere is at 10 km. that might not be always the case
c therefore i scale edheight with ZTROP/10km.	
	 	edheight(J)=edheight(J)*ZTROP/1e5/10. 
        enddo   
        close(64)

	k=1
	
c filling up the EDD array:	
        do J=1,NZ

c Zsom Rugheimer better way to determine K since otherwise K will advance ahead of the condition
c edheight(kk) <= Z(J) <= edheight(kk+1) (necessary for interpolation
c the kk that fulfills this condition will be the k

           do kk = 1,size(edheight)
               if((edheight(kk) .le. Z(J)).and.
     2                 (edheight(kk+1).ge.Z(J))) then
                   k=kk
               endif
           enddo

           	EDD(J) = edorig(k)+(edorig(k+1)-edorig(k))*
     2			(Z(J)-edheight(k))/(edheight(k+1)-edheight(k))	

		
c                if (Z(J).ge.edheight(k+1)) then
c                        k=k+1
c                endif


                if (EDD(J) .lt. 0) then
                write(*,*)(Z(J)-edheight(k))/(edheight(k+1)-edheight(k))
                  write(*,*) 'eddy is neg in atm.f K,J',K, J
                  write(*,*) EDD(J)
                  stop
                endif
		
c	 	write(*,*)Z(J),ZTROP/1e5,EDD(J)
       enddo
       
C   Initial constant mixing ratios used for present Earth
c A. Zsom: provided by the run_chemclim.py.
...
c        
c      DO i = 1, NZ
c        USOL(LH2, i) = USOL(LH2,i)*(FH2/USOL(LH2,1))
c         USOL(LH2,i) = FH2 
c       USOL(LCH4,i) = USOL(LCH4,i)*(FCH4/USOL(LCH4,1))
c        USOL(LCH4,i) = FCH4
c        USOL(LCO,i) = USOL(LCO,i)*(FCO/USOL(LCO,1))
c         USOL(LCO,i) = FCO
c        USOL(LN2O,i) = USOL(LN2O,i)*(FN2O/USOL(LN2O,1))
c          USOL(LN2O,i) = FN2O
c        USOL(LCH3CL,i) = USOL(LCH3CL,i)*(FCH3CL/USOL(LCH3CL,1))
c           USOL(LCH3CL,i) =FCH3CL
c       END DO
c        endif
...
c SR MacGregor Old fluxes
c         SGFLUX(9) = -7.2E9     
c         SGFLUX(10) = 1.643e11
c         SGFLUX(11) = 2.858E11
c         SGFLUX(14) = 1.131E9
c         SGFLUX(23) = 4.117E8
c S Rugheimer New fluxes for modern Earth/Sun with 300 layers, 0.2 albedo, tsurf=288.98
c         SGFLUX(9) = -3.32E9     
c         SGFLUX(10) = 1.31e11
cc         SGFLUX(10) = 4.e9 abio flux used in Segura 2005
cc         SGFLUX(10) = 6.8e8
c         SGFLUX(11) = 2.30E11
c         SGFLUX(14) = 6.75E8
c         SGFLUX(23) = 3.31E8

c S Rugheimer Fluxes for Epoch 3 calculated according to Kaltenegger 2007
c         SGFLUX(9) = -9.72e12     
c         SGFLUX(10) = 2.36e13
c         SGFLUX(11) = -1.31e13
c         SGFLUX(14) = 2.66e9
c         SGFLUX(23) = 6.7e7     
c These fluxes maybe completely wrong for Epoch 3!! this had to be done through trial and error
         SGFLUX(9) = -9.05e12     
c         SGFLUX(10) = 2.19e13
         SGFLUX(10) = 2.08e13
         SGFLUX(11) = -5.40e12
...
c S Rugheimer Fluxes for Epoch 4 calculated according to Kaltenegger 2007
c         SGFLUX(9) = -2.89e12     
c         SGFLUX(10) = 6.4e12
c         SGFLUX(11) = -3.80e12
c         SGFLUX(14) = 1.76e9
...
C Read character array for P&L tables, "int.rates.out.dat"
      ...
      READ(61,200)CHEM
      CLOSE(61)
c     WRITE(215,201)(J,(CHEM(M,J),M=1,5),J=1,NR)
      do j=1,NSP
      NUML(j) = 0
      NUMP(j) = 0
      enddo
...
c	do J=1,NZ
c	 READ(64,245) Z(J),EDD(J)	
c	end do 
c 245  FORMAT(2(1PE11.3, 1X))
c      close(64)
...
C
      ...
      ISULF = 0
c H2 volcanic flux use when using fixed vdep
      VOLFLX = 8.6E9    !Volcanic flux H2 from Guzman-Marmolejo 2014 on abiotic methane
      CALL GRID
      DZ = Z(2) - Z(1)
...
c        print*,i,FLUX(i)
        ...
        do i=1,108
         FLUX(i)=FLUX(i)*FLUXFAC
        enddo  
      endif
      write(*,*) 'star Flux Check atmchem'
      write(*,*) STARR
      write(*,*) 'Flux',FLUX

** Water calculation

c      JTROP = ZTROP/DZ + 0.01
      CALL PSATRAT(H2O)
       DO 23 J=1,JTROP 
...
C-KK	handled consistently. The #s are imported from the climate model.
      ...
      CALL AERTAB
      NZ1 = NZ - 1
      HA = 1.38E-16*T(NZ)/(1.67E-24*28.*G)
...
c A. Zsom: bug hunting
	if (isnan(RHS(K))) then 
		write(*,*)'NAN detected in RHS: ',I,J
c SR writing NAN output to convergence.txt
                write(600,*)'NAN detected in RHS: ',I,J
		stop
	endif      
...
c A. Zsom: the goto 26 lines take out parts of the matrix. 
c if used properly, it can significantly speed up the calculation.
c if you simulate something new however, comment out all goto 26 lines.
      IF(NCOUPLE .lt. 8) THEN 
ccc         IF((I.EQ.LH2S)) GOTO 26
ccc         IF(I.EQ.LHS) GOTO 26
ccc        IF(I.EQ.LHSO) GOTO 26
cc        IF(I.EQ.LCH3CL) GOTO 26
c        IF(I.EQ.LH) GOTO 26
c        IF (I.EQ.LSO) GOTO 26
ccc         IF (I.EQ.LCL2O2) GOTO 26
c For AD Leo
ccc         IF(I.EQ.LH2) GOTO 26 
C-KK	Added for low-O2 environments
c- Chlorine compounds
ccc        IF(I.EQ.LCLONO2)GOTO 26 
cc        IF(I.EQ.LHCL) GOTO 26
cc        IF(I.EQ.LCL) GOTO 26
cc	 IF(I.EQ.LHOCL) GOTO 26
cc        IF(I.EQ.LCLO) GOTO 26
ccc	 IF(I.EQ.LCH3CL) GOTO 26

C- Hydrogen 
c 	 IF(I.EQ.LH2O2) GOTO 26
c	 IF(I.EQ.LHO2) GOTO 26
c	 IF(I.EQ.LOH) GOTO 26
C- Sulfur
c
ccc        IF(I.EQ.LH2SO4) GOTO 26
c	 IF(I.EQ.LHSO) GOTO 26
cc       IF(I.EQ.LSO2) GOTO 26
c	 IF(I.EQ.LH2S) GOTO 26
c	 IF(I.EQ.LHS) GOTO 26
cc        IF(I.EQ.LSO) GOTO 26
c- Nitrogen
ccc        IF(I.EQ.LN2O5) GOTO 26
cc        IF(I.EQ.LHNO2) GOTO 26
cc	 IF(I.EQ.LNO2) GOTO 26
cc        IF(I.EQ.LHO2NO2) GOTO 26
cc        IF(I.EQ.LNO3) GOTO 26
cc        IF(I.EQ.LHNO3) GOTO 26
cc	 IF(I.EQ.LNO) GOTO 26

c	 IF(I.EQ.LCH3OOH) GOTO 26
c	 IF(I.EQ.LH2CO) GOTO 26
c        IF(I.EQ.LCH3O2) GOTO 26	
c	 IF(I.EQ.LO3) GOTO 26
c	 IF(I.EQ.LO) GOTO 26
c NO, CLO, h2o2, n2o5, hso, ho2no2, ch3o2
      ENDIF
ccc         IF (I.EQ.LCL2O2) GOTO 26
...
C
  ...
  37  CONTINUE
c Zsom - it should write some error msg when it stops.  
      IF(INDEX.NE.0) then
      	write(*,*)'INDEX NE 0',N,INDEX
      	STOP
      endif   
...
C ***** END THE TIME-STEPPING LOOP *****
  ...
  22  CONTINUE
c SR can I commment out the next line?? it sometimes causes problems where NSTEPS is redefined to too low
c      if(ICOUPLe.eq.1) NSTEPS =N

      OPEN(unit=81,file= DIRINOUT//'/atm_composition.dat')
      WRITE(81,399) USOL,T,EDD,DEN,SO4AER,AERSOL,WFALL,RPAR
 399  FORMAT(1P8E12.5)
      close(81) 
       write(*,*) "NSTEPS = (11)",NSTEPS

c Transfer results to coupled model
      do i=1,NSP2
        IISPEC(i)=ISPEC(i)
      enddo
      do i=1,NQ
       ...
       LLBOUND(i)= LBOUND(i)
       do ii=1,NZ 
         XUSOL(i,ii)=USOL(i,ii)
       enddo
      enddo
      LLBOUND(NQT) = LBOUND(NQT)
      XFLOW(NQT) = FLOW(NQT)
      do i=1,NSP
       XTP(i)= TP(i)
       XTL(i)= TL(i)
...
c      dircoup = '../CHEMCLIM/IO'
c	write(*,*)dircoup

	OPEN(unit=19,file= dircoup//'/mixing_ratios.dat')
...
c---------------------------------------------------------------
      ...
      INCLUDE 'INCLUDECHEM/comRRATS.inc'
      COMMON/RRATS/RXTOT(NR,NZ)

      DIMENSION FUP(NQT),FLOW(NQT),CON(NQT),FLUXCH(NQT,NZ)
     2  ,ZF(NZ),ZKM(NZ)
C
      write(*,*) "NSTEPS = (10.1)",NSTEPS
...
c      OPEN(unit=86,file= DIRINOUT//'/h2omixing.out')
      DO I=1,NZ
...
C-AP
	...
	END DO
 352  CONTINUE
c A. Zsom: the format code or the nr. of output numbers were not correct
c      write(90,151) JCOLD, USOL(3,JCOLD)
c 151  FORMAT(/1X,I3,' FH2O AT COLD TRAP =',1PE10.3) 
      write(90,151) USOL(3,JCOLD)
 151  FORMAT(/1X,I3,' FH2O AT COLD TRAP =',1PE10.3)
      IF(N.LT.NSTEPS) RETURN

      write(*,*) "NSTEPS = (10.2)",NSTEPS
...
C
 ...
 900  FORMAT(1X,'Reaction rate profiles'/)
      ...
      K2 = K1 + IROW - 1
      IF (L.EQ.LR) K2 = NR
      WRITE(214,901) (K,K=K1,K2)
 901  FORMAT(/5X,'Z  Rx#',4X,10(I4,7X))
      DO 91 J=1,NZ
      ZKM(J) = Z(J)/1.E+5
  91  WRITE(214,902) ZKM(J),(RXTOT(K,J),K=K1,K2)
 902  FORMAT(1P11E11.3)
  90  CONTINUE
      write(*,*) "NSTEPS = (10.3)",NSTEPS
