...
c in order to read the NF that contain  the useful wavelengths
     ...
     & 2200.,2222.,2247.,2273.,2299.,2326.,2353.,2750./
...
C  Open the necessary datafiles
      ...
      open(unit=2, file='model3200-test_scaled.pdat')
      open(unit=3, file='model3200_surf-test.pdat',status='unknown')
...
C Calculation for the interval 2000A-2381 A
      is1 = 1
      is2 = 0
      do 13 l=1,NL1
      dwav1 = wavu1(l) - wavl1(l)
      do 14 i=is1,NF
...
C   alam(is1) and alam(is2) are both within interval l
 ...
 18   flux1(l) = flux1(l) + flx(i)
      flux1(l) = flux1(l)/datnum * dwav1
      flux2(l) = flux1(l)/datnum
 13    continue
...
C   alam(is1) and alam(is2) are both within interval l
 ...
 12      fluxw(l) = fluxw(l) + flx(i)
      fluxw(l) = fluxw(l)/datnum * dwav
       fluxw2(l) = fluxw(l)/datnum
 6    continue

      do i=1,NL1
        write(3,400) i,flux1(i),flux2(i),wavl1(i),wavu1(i)
      enddo
      write(3,*)
      do i=1,NL
        write(3,400) i,fluxw(i),fluxw2(i),wavl(i),wavu(i)
      end do
 400  format(1x,i3,2(2x,E11.4),2(2x,f7.0))
      close(2)
c Checking the normalization
c     open(unit=5, file='model3200_scaled.pdat')
c      do ii=1,5
c       read(5,*)
c      enddo
c      do i=1,NF
c       read(5,*) xlam1(i),flx1(i)
c      enddo
c Calculating the integrated flux (erg/cm^2/s) at 1 AU
c      sum_flux1 = 0.0
c      do i=1,NF-1
c         delta1= xlam1(i+1)-xlam1(i)
c         sum_flux1 = sum_flux1 + delta1*(flx1(i)+flx1(i+1))/2.
c      end do
c      facm1 = sum_flux1/(1.36e6)
