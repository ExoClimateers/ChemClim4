...
c in order to read the NF that contain  the useful wavelengths
      PARAMETER(NF=1123, NS =1, NL=38, NL1=16)
...
C  Open the necessary datafiles
      open(unit=1, file='model_T3650_logg5.3_list.dat' , status='old')
      open(unit=2, file='model3650_scaled.pdat', status='unknown')
      open(unit=3, file='model3650_surf.pdat', status='unknown')
c Writing the headers on the output files
c NOTE: Change the headers according to the star
      write(2,202)
 202  format(1x,'T3650 FLUX',' scaled at the distance the planet')
      write(3,401)
 401  format(3x,'L',6x,'T3650')
...
c Data for AD Leo (M star) d=4.9 pc
       dstar = 0.0
       dfac = (206265.*dstar)**2
...
c Data for model star Teff=3650, L=4.3e-2 L_sol
c Data for model star Teff=3400, L=2.2e-2 L_sol
c Data for model star Teff=3200, L=9.45e-3 L_sol
        xLumstar= 4.3e-2
C   Fluxes must be in ergs/cm^2/s/A
      ...
      do i=1,NF
       read(1,*) xlam(i),flx(i)
c        xlam(i)= xlam(i)*1.e4      !converting from um to A
          xlam(i)=xlam(i)*10.       !converting nm to A
c        flx(i)= flx(i)*1.e3        !W/cm^2/um to erg/cm^2/s/A
         flx(i)= flx(i)*1.e2        !W/m^2/nm to erg/cm^2/s/A   
      end do
...
c Checking the normalization
      open(unit=5, file='model3650_scaled.pdat')
      do ii=1,5
       read(5,*)
      enddo
      do i=1,NF
