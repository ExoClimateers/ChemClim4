...
for l in range(len(fluxfac)):  # loop over orbital distance flux scaling
    for j in range(len(star)): # loop over stellar type
        ...
        for k in range(len(ND)):              # loop over altitude grid
            if ND[k]>600:                     # check for excessive altitude values
        ...
        for k in range(len(ND)):                                       # loop over altitude grid
            ...
            '''
                # TODO:: we can save alot of lines by not having large if blocks and instead saying 
                if name == " ": 
                    namek = str(ND[k])
                else:
                    namek = name+"_"+str(ND[k])
                # TODO:: then we use namek instead of name+str(ND[k])
            '''
            ...
            if name == " ":
                os.system("cp IO/outchem.dat IO/outchem_"+str(ND[k])+"_ff"+str(fluxfac[l])+".dat\n")
                os.system("cp IO/output_couple.dat IO/output_couple_"+str(ND[k])+"_ff"+str(fluxfac[l])+".dat")
                os.system("cp IO/altPT-lisa.dat IO/altPT_"+str(ND[k])+"_ff"+str(fluxfac[l])+".txt")
                os.system("cp IO/chem-lisa.dat IO/chem_"+str(ND[k])+"_ff"+str(fluxfac[l]))
                os.system("cp IO/convergence.txt IO/convergence_"+str(ND[k])+"_ff"+str(fluxfac[l])+".txt")
            else:
                os.system("cp IO/outchem.dat IO/outchem_"+name+"_"+str(ND[k])+"_ff"+str(fluxfac[l])+".dat\n")
                os.system("cp IO/output_couple.dat IO/output_couple_"+name+"_"+str(ND[k])+"_ff"+str(fluxfac[l])+".dat")
